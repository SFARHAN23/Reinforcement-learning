"""
Train_CarRace_TD3AM_GT3.py  —  Monza, all-corner curriculum
=============================================================
RESUME_FROM : path to checkpoint, or None to start fresh
SPAWN_BLOCK : episodes per spawn mode

Spawn rotation (11 corners + 1 RAND, no FIXED):
    block 0  : T1          spawn@900m,  spd=7.9  m/s
    block 1  : T2          spawn@927m,  spd=7.9  m/s
    block 2  : Curva Grande spawn@1011m, spd=12.0 m/s
    block 3  : Roggia L    spawn@1961m, spd=9.6  m/s
    block 4  : Roggia R    spawn@2474m, spd=9.6  m/s
    block 5  : Lesmo 1     spawn@2525m, spd=12.0 m/s
    block 6  : Lesmo 2     spawn@3046m, spd=12.0 m/s
    block 7  : Ascari L    spawn@3352m, spd=12.0 m/s
    block 8  : Ascari M    spawn@4420m, spd=12.0 m/s
    block 9  : Ascari X    spawn@4541m, spd=12.0 m/s
    block 10 : Parabolica  spawn@4665m, spd=12.0 m/s
    block 11 : RAND
    → cycle length = 12 × SPAWN_BLOCK episodes

Key changes from previous version (_C2_R):
    1. Spawn AT corner entry (3m before), NOT 33m before
       Previous 33m run-up → policy accelerated → arrived above v_max → crash
    2. All 11 Monza corners covered, not just T1
    3. FIXED removed from rotation (was poisoning Q-values at T1)
    4. TD3.py now saves/loads critic → true seamless resume
"""
...
from curriculum import (
    ...
    MONZA_CORNERS, MONZA_FINE_TUNE_APPROACHES,
    next_corner_complex_exit, wrapped_trip_distance
)

# ═══════════════════════════════════════════════════════════════════════════
RESUME_FROM = None  # auto-resume from the latest checkpoint
SPAWN_BLOCK = 20      # short blocks → full circuit covered quickly
FINE_TUNE_BLOCK = 10
FINE_TUNE_AFTER = 200_000
...
curriculum_get_spawn, spawn_params = build_spawn_schedule(MONZA_CORNERS, SPAWN_BLOCK)
finetune_get_spawn, finetune_params = build_finetune_schedule(
    MONZA_FINE_TUNE_APPROACHES, FINE_TUNE_BLOCK,
...
if RESUME_FROM is not None:
    ...
    print(f"  ✓ Loaded '{RESUME_FROM}'  critic={'yes' if critic_loaded else 'no (old ckpt — actor only)'}")
    ...
    if os.path.exists(lp):
        ...
        print(f"  ✓ Trainlog ({len(trainlog)} eps)")
        if trainlog:
...
n_corners = len(MONZA_CORNERS)
print(f"\n{'='*64}")
print(f"  Monza GT3 — all-corner curriculum")
print(f"  Circuit : {env.track.total_trip:.0f} m  |  Corners : {n_corners}")
print(f"  Cycle   : {n_corners} corners + 1 RAND × {SPAWN_BLOCK} eps")
...
for trip, spd, label, vmax in spawn_params:
    print(f"    {label:<16} @{trip:.0f}m  {spd:.1f}m/s ({spd/vmax*100:.0f}% v_max)")
for mode_name, trip, spd, label, vmax, dist in finetune_params:
    print(f"    FT {mode_name[:3].upper():<3} {label:<10} @{trip:.0f}m  "
          f"{spd:.1f}m/s  ({dist:.0f}m run-up)")
print(f"{'='*64}\n")
...
for episode in range(100_000):
    ...
    progress = max_lap  # environment already tracks relative lap_pct perfectly now
